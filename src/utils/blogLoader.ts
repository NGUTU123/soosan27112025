...
// Tự động load tất cả bài viết từ thư mục tin-tuc-nganh-van-tai
const loadTinTucNganhVanTaiPosts = async (): Promise<BlogPost[]> => {
  try {
    const modules = import.meta.glob('/src/data/blog-posts/tin-tuc-nganh-van-tai/*.ts', { eager: true });
    ...
    console.error('Error loading tin tuc nganh van tai posts:', error);
    return [];
  }
};
...
export const loadAllBlogPosts = async (): Promise<{
  tinTucNganhVanTaiPosts: BlogPost[];
